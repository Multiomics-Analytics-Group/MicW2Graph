...
def plot_histogram(
    data, column, x_title, y_title, nbins, color_palette
) -> px.histogram:
    """
    ...
    hist_plot = px.histogram(
        data, x=column, nbins=nbins, opacity=0.8, color_discrete_sequence=color_palette
    )
...
def plot_bar(
    data, x_column, color_column, legend_title, x_title, y_title, color_palette
) -> px.histogram:
    """
...
def plot_two_pies(
    labels, values1, values2, title1, title2, legend_title, color_palette
) -> go.Figure:
    """
...
def plot_stacked_bar_allbiomes(
    abund_dfs, tax_dfs, biome_names, color_palette
) -> px.bar:
    """
    ...
    bar_plot = px.bar(
        top_species_df_all_biomes,
        x="Biome",
        y="Relative Abundance",
        color="Species",
        category_orders={"Species": top_species_df_all_biomes["Species"].unique()},
        opacity=0.8,
        color_discrete_sequence=color_palette,
    )
...
def plot_stacked_bar_bybiome(abund_df, tax_df, study_ids) -> px.bar:
    ...
    palette_hex = [
        "#" + "".join([f"{int(c*255):02x}" for c in rgb])
        for rgb in cc.glasbey_bw_minc_20
    ]
    ...
    top_taxa_plot = px.bar(
        top_species_df,
        x="Study",
        y="Relative Abundance",
        color="Species",
        category_orders={"Species": top_species_df["Species"].unique()},
        opacity=0.8,
...
def plot_pcoa(
    ...
    if type_input_data in ["all_biomes", "biome"]:
        ...
        for column in additional_columns:
            bc_pcoa_data = bc_pcoa_data.merge(
                studies_data[["study_id", column]], on="study_id"
            )
        ...
        if type_input_data == "biome":
            ...
            bc_pcoa_data["specific_biome"] = np.where(
                bc_pcoa_data["specific_biome"] == "NA - NA",
                bc_pcoa_data["biome"],
                bc_pcoa_data["specific_biome"],
            )
        ...
        bc_pcoa_data["study_id"] = bc_pcoa_data["study_id"].str.cat(
            bc_pcoa_data["biome"], sep=" - "
        )
    ...
    if "experiment_type" in bc_pcoa_data.columns:
        bc_pcoa_data["experiment_type"] = bc_pcoa_data[
            "experiment_type"
        ].str.capitalize()

    # Make the plot
    pcoa_plot = px.scatter(
        bc_pcoa_data,
        x="PC1",
        y="PC2",
        opacity=0.8,
    ...
    pcoa_plot.update_traces(marker=dict(size=7)).update_layout(
        xaxis=dict(
...
def plot_pcoa_stud_comp(bc_mat, abund_df, studies_data, study_ids) -> px.scatter:
    ...
    bc_pcoa_data = bc_pcoa_data.merge(
        studies_data[["study_id", "biome"]], on="study_id"
    )
...
def plot_bar_top10taxa(abund_table, tax_rank) -> px.bar:
    ...
    if tax_rank == "Phylum":
        ...
        abund_table_top10 = abund_table_reset.melt(
            id_vars="phylum", var_name="assembly_run_ids", value_name="count"
        )
        # Rename the column
        abund_table_top10 = abund_table_top10.rename(columns={"phylum": "taxa"})
    elif tax_rank == "Genus":
        abund_table_top10 = abund_table_reset.melt(
            id_vars="Genus", var_name="assembly_run_ids", value_name="count"
        )
        abund_table_top10 = abund_table_top10.rename(columns={"Genus": "taxa"})
    elif tax_rank == "Species":
        abund_table_top10 = abund_table_reset.melt(
            id_vars="Genus_Species", var_name="assembly_run_ids", value_name="count"
        )
    ...
    abund_table_top10 = abund_table_top10.sort_values(
...
def load_and_preprocess_net(
    graphml_file, color_palette=px.colors.qualitative.Dark24, min_size=6, max_size=30
) -> nx.Graph:
    """
    ...
    for node in G.nodes():
        ...
        G.nodes[node]["color"] = color_map.get(
            G.nodes[node].get("cluster")
        )  # Default to green if no cluster color

    return G


def load_and_preprocess_kg(
    graphml_file, node_colors, edge_colors, min_size=10, max_size=50
) -> nx.Graph:
    """
    ...
    for node, data in G.nodes(data=True):
        ...
        if betweenness_centrality == min_betweenness:
            ...
            size = min_size + (max_size - min_size) * (
                (betweenness_centrality - min_betweenness)
...
def footer():
    ...
    st.markdown(
        "The code for this project is available under the [MIT License](https://mit-license.org/) in this [GitHub repo](https://github.com/Multiomics-Analytics-Group/MicW2Graph). If you use or modify the source code or the outputs of this project, cite this work using the following DOI: [![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.11394618.svg)](https://doi.org/10.5281/zenodo.11394618)"
    )
