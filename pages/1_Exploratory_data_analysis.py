...
studies_data = utils.load_studies_data(
    "data/EDA/wwtstudies_greater_mediantaxa_perbiome_species.csv"
)
...
if eda_type == "Summary of all studies":
    # Display title
    st.markdown(
        "<h3 style='text-align: center; color: black;'>Exploratory data analysis for all studies</h3>",
        unsafe_allow_html=True,
    )

    # Create plot the number of studies per studies
    st.markdown(
        "<h4 style='text-align: left; color: black;'>Number of samples per study</h4>",
        unsafe_allow_html=True,
    )
    hist_samples = web_utils.plot_histogram(
        studies_data,
        "n_samples",
        "Number of samples",
        "Number of studies",
    ...
    st.markdown(
        "<h4 style='text-align: left; color: black;'>Sampling countries for all studies</h4>",
        unsafe_allow_html=True,
    )
    pie_plot_countries = web_utils.plot_pie(
        studies_data, "sampling_country", "Country", px.colors.qualitative.Dark24
    )
    ...
    st.markdown(
        "<h4 style='text-align: left; color: black;'>Data types for all studies</h4>",
        unsafe_allow_html=True,
    )
    bar_plot_dtypes = web_utils.plot_bar(
        studies_data,
        "experiment_type",
        "biome",
        "Biome",
    ...
    abund_df_species_wwt = pd.read_csv(
        merged_files_per_biome["Wastewater"]["abundance"], index_col=0
    )
    tax_df_species_wwt = pd.read_csv(
        merged_files_per_biome["Wastewater"]["taxonomy"], index_col=0
    )

    abund_df_species_wwt_ws = pd.read_csv(
        merged_files_per_biome["Wastewater_Water_and_sludge"]["abundance"], index_col=0
    )
    tax_df_species_wwt_ws = pd.read_csv(
        merged_files_per_biome["Wastewater_Water_and_sludge"]["taxonomy"], index_col=0
    )

    abund_df_species_wwt_ind = pd.read_csv(
        merged_files_per_biome["Wastewater_Industrial_wastewater"]["abundance"],
        index_col=0,
    )
    tax_df_species_wwt_ind = pd.read_csv(
        merged_files_per_biome["Wastewater_Industrial_wastewater"]["taxonomy"],
        index_col=0,
    )

    abund_df_species_wwt_as = pd.read_csv(
        merged_files_per_biome["Wastewater_Activated_Sludge"]["abundance"], index_col=0
    )
    tax_df_species_wwt_as = pd.read_csv(
        merged_files_per_biome["Wastewater_Activated_Sludge"]["taxonomy"], index_col=0
    )

    # Create pie plot for the number of studies and samples per biome
    st.markdown(
        "<h4 style='text-align: left; color: black;'>Number of studies and samples per biome</h4>",
        unsafe_allow_html=True,
    )
    ...
    samples_per_biome = [
        abund_df_species_wwt_as.shape[1],
        abund_df_species_wwt.shape[1],
        abund_df_species_wwt_ws.shape[1],
        abund_df_species_wwt_ind.shape[1],
    ]

    # Create the pie plots
    pie_plots_biomes = web_utils.plot_two_pies(
        biomes,
        studies_per_biome.values,
        samples_per_biome,
        "Studies",
        "Samples",
        "Biomes",
    ...
    st.markdown(
        "<h4 style='text-align: left; color: black;'>Top 5 species by biome</h4>",
        unsafe_allow_html=True,
    )

    # Prepare the lists of dataframes and biome names
    abund_dfs = [
        abund_df_species_wwt,
        abund_df_species_wwt_ws,
        abund_df_species_wwt_ind,
        abund_df_species_wwt_as,
    ]
    tax_dfs = [
        tax_df_species_wwt,
        tax_df_species_wwt_ws,
        tax_df_species_wwt_ind,
        tax_df_species_wwt_as,
    ]
    biome_names = [
        "Wastewater",
        "Water and sludge",
    ...
    top_species_plot_biome, abund_tax_merged = web_utils.plot_stacked_bar_allbiomes(
        abund_dfs, tax_dfs, biome_names, px.colors.qualitative.Dark24
    )
    ...
    st.markdown(
        "<h4 style='text-align: left; color: black;'>PCoA plots of samples from all studies using Bray-Curtis dissimilarity matrices</h4>",
        unsafe_allow_html=True,
    )
    ...
    bc_mat, samples_df = utils.calculate_bray_curtis_dist(
        abund_df, "all_biomes", sample_info
    )
    ...
    # Show the PCoA plot
    pcoa_plot_all_studies = web_utils.plot_pcoa(
        bc_mat,
        samples_df,
        studies_data,
        color_var,
    ...
    st.markdown(
        "<h4 style='text-align: left; color: black;'>Sample information for all studies</h4>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<h4 style='text-align: left; color: black;'>Abundance data for all studies</h4>",
        unsafe_allow_html=True,
    )
    ...
    column_order = ["Species"] + [
        col for col in abund_table_with_index.columns if col != "Species"
    ]
    ...
    # Title for the page
    st.markdown(
        "<h3 style='text-align: center; color: black;'>Exploratory data analysis for studies by biome</h3>",
        unsafe_allow_html=True,
    )
    ...
    sample_info = pd.read_csv(
        f"data/EDA/Samples_metadata/Merged_tables/Biomes/{biome}_merged_samples_metadata.csv"
    )
    abund_df = pd.read_csv(
        f"data/EDA/Abundance_tables/Merged_tables/Biomes/{biome}/{biome}_merged_abund_tables_species.csv",
        index_col=0,
    )
    tax_df = pd.read_csv(
        f"data/EDA/Abundance_tables/Merged_tables/Biomes/{biome}/{biome}_merged_taxa_tables_species.csv",
        index_col=0,
    )
    study_ids = pd.read_csv(
        f"data/EDA/Abundance_tables/Merged_tables/Biomes/{biome}/{biome}_studies_per_sample.csv",
        index_col=0,
    )

    # Calculate Bray-Curtis dissimilarity matrix and preprocess sample info
    bc_mat, samples_df = utils.calculate_bray_curtis_dist(
        abund_df, "biome", sample_info
    )

    # Create a violin plot to show the distribution of the distances within and between the studies
    st.markdown(
        "<h4 style='text-align: left; color: black;'>Distribution of the Bray-Curtis distances</h4>",
        unsafe_allow_html=True,
    )
    ...
    st.warning(
        "Some studies have just a few samples, so they are not shown in this plot",
        icon="⚠️",
    )

    # Create a barplot to show the top 5 species per study
    st.markdown(
        "<h4 style='text-align: left; color: black;'>Top 5 species per study</h4>",
        unsafe_allow_html=True,
    )
    top_species_plot, abund_df_merged = web_utils.plot_stacked_bar_bybiome(
        abund_df, tax_df, study_ids
    )
    st.plotly_chart(top_species_plot, use_container_width=True)

    # Create PCoA plots by biome for all studies
    st.markdown(
        f"<h4 style='text-align: left; color: black;'>PCoA plots of samples from the {biome} biome using Bray-Curtis dissimilarity matrices</h4>",
        unsafe_allow_html=True,
    )
    ...
    # Show the PCoA plot
    pcoa_plot_biome = web_utils.plot_pcoa(
        bc_mat, samples_df, studies_data, color_var, color_option, "biome", study_ids
    )
    ...
    st.markdown(
        f"<h4 style='text-align: left; color: black;'>Abundance table for the {biome_title} biome</h4>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        f"<h4 style='text-align: left; color: black;'>Sample table for the {biome_title} biome</h4>",
        unsafe_allow_html=True,
    )
    web_utils.display_dataframe_with_aggrid(sample_info)
    ...
    # Title for the page
    st.markdown(
        "<h3 style='text-align: center; color: black;'>Exploratory data analysis for a specific study</h3>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        f"<h4 style='text-align: left; color: black;'>Summary of the {selected_study} study</h4>",
        unsafe_allow_html=True,
    )
    ...
    if len(sample_info) > 2:
        ...
        st.markdown(
            f"<h4 style='text-align: left; color: black;'>Distribution of the Bray-Curtis distances for the {selected_study} study</h4>",
            unsafe_allow_html=True,
        )

        # Calculate Bray-Curtis dissimilarity matrix and preprocess sample info
        bc_mat, samples_df = utils.calculate_bray_curtis_dist(
            abund_table, "study", sample_info, tax_rank
        )
        ...
        st.markdown(
            f"<h4 style='text-align: left; color: black;'>Top 10 most abundant {tax_rank} in the {selected_study} study</h4>",
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            f"<h4 style='text-align: left; color: black;'>PCoA plot of samples from the {selected_study} study at {tax_rank} using Bray-Curtis dissimilarity matrices</h4>",
            unsafe_allow_html=True,
        )
        pcoa_plot_biome = web_utils.plot_pcoa(
            bc_mat, samples_df, studies_data, "biome", "Biome", "study"
        )
        st.plotly_chart(pcoa_plot_biome, use_container_width=True)

        # Display abundance data for the selected study
        st.markdown(
            f"<h4 style='text-align: left; color: black;'>Abundance data for the {selected_study} study at {tax_rank} level</h4>",
            unsafe_allow_html=True,
        )
        ...
        # Ensure the index column is the first one
        column_order = [index_name] + [
            col for col in abund_table_with_index.columns if col != index_name
        ]
        abund_table_with_index = abund_table_with_index[column_order]
        ...
        st.markdown(
            f"<h4 style='text-align: left; color: black;'>Sample table for the {selected_study} study at {tax_rank} level</h4>",
            unsafe_allow_html=True,
        )
        web_utils.display_dataframe_with_aggrid(sample_info)
        ...
        st.warning(
            "The selected study has less than two samples, so the PCoA plot and the distribution of the Bray-Curtis distances cannot be created.",
            icon="⚠️",
        )
    ...
    # Title for the page
    st.markdown(
        "<h3 style='text-align: center; color: black;'>Exploratory data analysis for comparison between studies</h3>",
        unsafe_allow_html=True,
    )

    # Create a selectbox to choose the taxonomic rank
    tax_rank = st.selectbox("Taxonomic Rank:", options=["Phylum", "Genus", "Species"])
    ...
    # Show the df
    st.markdown(
        "<h4 style='text-align: left; color: black;'>List of studies and their biomes</h4>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        f"<h4 style='text-align: left; color: black;'>Comparison between studies at {tax_rank} level</h4>",
        unsafe_allow_html=True,
    )

    # Create a selectbox to choose the two studies
    selected_studies = st.multiselect(
        "Selected studies:", studies_biomes["Study ID"].unique()
    )
    ...
    if len(selected_studies) == 0:
        ...
        study1_info, sample1_info = utils.load_study_data(
            studies_data, selected_studies[0]
        )
        study1_info = study1_info.T
        study1_info = study1_info.drop(index="study_id")
        study1_info.columns = [selected_studies[0]]

        study2_info, sample2_info = utils.load_study_data(
            studies_data, selected_studies[1]
        )
        ...
        studies_info = pd.merge(
            study1_info, study2_info, left_index=True, right_index=True
        )
        ...
        if tax_rank == "Phylum":
            abund_table1 = utils.load_abund_table(
                selected_studies[0], tax_rank="phylum"
            )
            abund_table1 = utils.preprocess_abund_table(abund_table1, tax_rank="phylum")
            abund_table2 = utils.load_abund_table(
                selected_studies[1], tax_rank="phylum"
            )
            ...
            abund_table1 = utils.load_abund_table(
                selected_studies[0], tax_rank="species"
            )
            abund_table1 = utils.preprocess_abund_table(
                abund_table1, tax_rank="species"
            )
            abund_table2 = utils.load_abund_table(
                selected_studies[1], tax_rank="species"
            )
            abund_table2 = utils.preprocess_abund_table(
                abund_table2, tax_rank="species"
            )
        ...
        st.markdown(
            f"<h4 style='text-align: left; color: black;'>PCoA plot of samples from the {selected_studies[0]} and {selected_studies[1]} studies at {tax_rank} level using Bray-Curtis dissimilarity matrices</h4>",
            unsafe_allow_html=True,
        )

        # Calculate Bray-Curtis dissimilarity matrix and preprocess sample info
        bc_mat, samples_df = utils.calculate_bray_curtis_dist(
            merged_df, "comparison", tax_rank=tax_rank
        )

        # Show the PCoA plot
        pcoa_plot_comp = web_utils.plot_pcoa_stud_comp(
            bc_mat, merged_df, studies_data, study_ids
        )
        st.plotly_chart(pcoa_plot_comp, use_container_width=True)

        # Show the merged abundance table
        st.markdown(
            f"<h4 style='text-align: left; color: black;'>Abundance table for the {selected_studies[0]} and {selected_studies[1]} studies at {tax_rank} level</h4>",
            unsafe_allow_html=True,
        )
