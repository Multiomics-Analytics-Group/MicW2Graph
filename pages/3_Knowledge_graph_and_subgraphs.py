...
with col2:
    ...
    # Selectboxes to choose the sub-biome and experiment type
    with subcol1:
    ...
    # Use a flag to control rendering of the graph
    render_graph = False
    if sub_biome and exp_type:
        ...
        try:
            ...
            # Catch specific error for more informative message
            st.warning(
                "The KG of the sub-biome and experiment type is not aviáilable. Select another combination.",
                icon="⚠️",
            )
        except FileNotFoundError:
            # Catch specific error for more informative message
            st.warning(
                "The knowledge graph file could not be found. Please select another sub-biome and experiment type.",
                icon="⚠️",
            )
        except Exception as e:
            # Catch other exceptions
            st.warning(f"An unexpected error occurred: {e}", icon="⚠️")
    else:
        st.warning("Please select a sub-biome and experiment type.", icon="⚠️")

    # Only render the graph and related components if the data was loaded successfully
    if render_graph:
        # Initiate PyVis network object
        net = Network(
            height="600px", width="100%", bgcolor="white", font_color="#555555"
        )
