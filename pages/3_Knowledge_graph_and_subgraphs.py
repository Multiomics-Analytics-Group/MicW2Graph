...
st.markdown("<h3 style='text-align: center; color: black;'>Metagraph and visualization of the KG for all sub-biomes and experiment types</h3>", unsafe_allow_html=True)
...
with col2:
    ...
    st.markdown("<h3 style='text-align: center; color: black;'>MicW2Graph subgraphs</h3>", unsafe_allow_html=True)
    subcol1, subcol2 = st.columns(2)

    with subcol1:
        # Selectbox to choose the sub-biome to explore
        sub_biome = st.selectbox(" ", 
                                 ["Activated sludge", "Wastewater", "Industrial wastewater", "Water and sludge"],
                                 index = None,
    ...
    try:         
        ...
        kg_subgraph_path = st.session_state.config['knowledge_graphs'][sub_biome][exp_type]
        ...
        G = web_utils.load_and_preprocess_kg(kg_subgraph_graphml, color_nodes, edge_colors)
    except:
    ...
    if control_layout:
        net.force_atlas_2based(gravity=-50, central_gravity=0.005, spring_length=100, spring_strength=0.08, damping=0.4)
