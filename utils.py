...
def load_studies_data(input_file) -> pd.DataFrame:
    ...
    all_data = all_data.drop(
        all_data.columns[all_data.columns.str.contains("unnamed", case=False)], axis=1
    )
...
def calculate_bray_curtis_dist(
    abund_df, type_input_data, sample_info=None, tax_rank=None
):
    """
    ...
    if type_input_data == "comparison":
        ...
        abund_table_merged_mat = (
            abund_df.apply(pd.to_numeric, errors="coerce").fillna(0).to_numpy()
        )
    ...
    abund_df_reshaped = abund_df.reset_index().melt(
        id_vars=id_var, var_name="assembly_run_ids", value_name="count"
    )
    ...
    samples_df = pd.merge(
        sample_info_exploded,
        abund_df_reshaped,
        left_on="assembly_run_ids",
        right_on="assembly_run_ids",
    )
    ...
    if type_input_data == "biome":
        ...
        samples_df = samples_df[
            ["assembly_run_ids", "sample_id", "biome_feature", "biome_material"]
        ...
        samples_df["biome"] = (
            samples_df["biome_feature"] + " - " + samples_df["biome_material"]
        )
    ...
    # Obtain Bray-Curtis distance matrix
    bc_mat = beta_diversity(
        "braycurtis", abund_table_mat, samples_df["assembly_run_ids"]
    )
...
def load_study_data(all_data, selected_study):
    ...
    sample_info = pd.read_csv(
        f"data/EDA/Samples_metadata/{selected_study}/{selected_study}_samples_metadata.csv"
    )
...
def load_abund_table(selected_study, tax_rank):
    ...
    if filtered_files:
        ...
        print(
            f"No files found for the study '{selected_study}' in folder '{folder_path}'."
        )
        return None
